...
import { MdOutlineKeyboardArrowDown, MdOutlineKeyboardArrowUp, MdCheckCircleOutline, MdTruck, MdEngineering, MdPerson, MdWbSunny, MdWarning, MdInventory, MdAccessTime } from "./Icons"; 
import { FiTruck } from "./Icons"; 
...
const RecommendationCard = ({ recommendation, index, onScenarioSelect }) => {
  ...
  return (
    <div className="card !border-gray-600 !px-4 min-w-[280px] max-w-xs flex flex-col justify-between shrink-0">
      <div>
  ...
  return (
    <div className="w-full text-left card !border-none !py-6 !px-4 mb-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
        <p className="note !text-xs flex justify-between sm:justify-end gap-x-4 items-center"><span>Target:</span> <span className="small-text">{apiData.target_tonnage || "N/A"} Ton</span></p>
        <p className="note !text-xs flex justify-between sm:justify-end gap-x-4 items-center"><span>Hasil Prediksi:</span> <span className="body-text !text-xs !text-red-500">{apiData.initial_prediction} Ton</span></p>
        <p className="note !text-xs flex justify-between sm:justify-end gap-x-4 items-center"><span>Konfigurasi:</span><span className="small-text">{isShipping ? `Stock: ${apiData.stock}, Trans: ${apiData.transportCapacity}` : `${apiData.truckCount} Truk, ${apiData.excavatorCount} Ekskavator`}</span></p>
        <p className="note !text-xs flex justify-between sm:justify-end gap-x-4 items-center"><span>&nbsp;</span><span className="small-text">{isShipping ? `Loading: ${apiData.loadingTime} Jam, ${weatherInfo.label}` : `${apiData.operatorCount} Operator, ${weatherInfo.label}`}</span></p>
      </div>
    </div>
  );
};
      ...
      <div className="p-0 rounded-xl">
        ...
        <div className={`overflow-hidden transition-all duration-500 ease-in-out w-full ${isOpen ? "max-h-[3000px] opacity-100 p-6" : "max-h-0 opacity-0 p-0"} bg-card/50 border border-t-0 border-[#EEEEEE] rounded-b-xl`}>
          ...
          {/* PERBAIKAN RESPONSIVE DI SINI:
              1. grid-cols-1 sm:grid-cols-3: Di HP 1 kolom (turun ke bawah), di Tablet/Desktop 3 kolom.
              2. Text alignment: Di HP rata kiri (atau justify-between), di Desktop rata tengah.
          */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 card !border-gray-600 !py-6 !px-4 rounded-lg mb-6">
            <div className="flex justify-between sm:block text-left sm:text-center border-b sm:border-b-0 border-gray-700 pb-2 sm:pb-0">
              <p className="small-text">Target</p>
              <p className="body-text">{target_tonnage} Ton</p>
            </div>
            <div className="flex justify-between sm:block text-left sm:text-center border-b sm:border-b-0 border-gray-700 pb-2 sm:pb-0">
              <p className="small-text">Prediksi Saat Ini</p>
              <p className="body-text !text-yellow-400">{initial_prediction} Ton</p>
            </div>
            <div className="flex justify-between sm:block text-left sm:text-center">
              <p className="small-text">Gap (Kekurangan)</p>
              <p className="body-text !text-red-500">{initial_difference} Ton</p>
            </div>
          </div>

          <div className="border-t border-gray-700 mt-6 pt-4">
            <p className="note mb-4">Pilih salah satu skenario di atas atau klik 'Finalisasi dan Kirim' jika Anda puas.</p>
            {/* Flex-col di HP agar tombol tidak gepeng */}
